...
def compare_blocks(correct_block, your_block, m, diag):
    ...
    if diag:
        correct_mat = np.frombuffer(correct_block, dtype=f'uint{8*args.size}').reshape((m, m))
        your_mat = np.frombuffer(your_block, dtype=f'uint{8*args.size}').reshape((m, m))
...
def compare_files(standard_output, submission_output):
    ...
    if n1 != n2 or m1 != m2:
        ...
        return 0, log
    elif file_size != 12 + k2 * (args.size * m2**2 + 8):
        log = f"file_size({file_size}) != 12 + k*({args.size} * m**2 + 8)({12 + k2 * (args.size * m2**2 + 8)})"
        return 0, log
    n, m = n1, m1
